...
首页现在默认进入员工端，右上角通过“演示角色”下拉框切换员工端、企业管理端和高级配置；浏览器前进、后退与旧管理链接继续可用。
...
当前使用模拟员工身份和单企业数据。下拉框仅用于演示视角切换，左侧导航和页面内容随当前视角变化，不代表认证或权限隔离；真实账号、租户隔离与角色授权仍需单独实现。员工待办的完成状态来自现有业务记录，首版不提供员工自行确认完成的入口。处理结果通过员工刷新页面查看，尚无外部消息推送。

...
export default function KnowledgeCenter({ documents, stats, importJobs, runtime, request, onReload, notify }: { documents: KnowledgeDocument[]; stats: KnowledgeStats; importJobs: KnowledgeImportJob[]; runtime: KnowledgeRuntime; request: Request; onReload: () => Promise<void>; notify: (message: string) => void }) {
  ...
  return <div className="page knowledge-center rag-center">
    <div className="page-heading"><div><h1>知识库管理</h1><p>上传 PDF、PPTX、DOCX 或文本文件，系统自动解析；复核发布后完成切片和混合检索索引。手工内容保存后立即参与 Agent 检索。</p></div><div className="heading-actions"><button className="secondary compact" onClick={reset}>＋ 手工录入</button><button className="primary compact" disabled={uploading} onClick={() => fileRef.current?.click()}>{uploading ? "正在解析…" : "＋ 上传知识文档"}</button></div></div>
    ...
    <div className="rag-runtime panel"><div><span className="rag-dot"/><strong>RAG 索引运行中</strong><small>{runtime.embedding?.label || runtime.embedding?.model} · {runtime.embedding?.semantic ? "语义向量" : "本地字符向量（可切换外部 Embedding）"}</small></div><div className="ingestion-pipeline" aria-label="知识入库流程"><span>上传</span><i>→</i><span>解析</span><i>→</i><span>人工复核</span><i>→</i><span>发布索引</span></div></div>
    <div className="dashboard-metrics knowledge-metrics rag-metrics"><div className="panel"><b>{stats.active}</b><span>已发布文档</span><small>{stats.total} 份文档总量</small></div><div className="panel"><b>{stats.chunks || 0}</b><span>可检索切片</span><small>{stats.categories} 个知识分类</small></div><div className={`panel ${(stats.review || 0) + (stats.failed || 0) > 0 ? "needs-attention" : "healthy"}`}><b>{(stats.review || 0) + (stats.failed || 0)}</b><span>待处理文档</span><small>{stats.review || 0} 待复核 · {stats.failed || 0} 失败</small></div><div className="panel"><b>{recentJobs[0]?.status === "completed" ? "完成" : recentJobs[0]?.status === "failed" ? "失败" : recentJobs[0] ? `${recentJobs[0].progress || 0}%` : "—"}</b><span>最近导入</span><small>{recentJobs[0]?.filename || "暂无导入任务"}</small></div></div>
    ...
    {recentJobs.length > 0 && <details className="panel import-jobs import-jobs-collapsible"><summary><div><strong>最近导入任务</strong><small>原文件解析、复核与索引进度</small></div><span>{recentJobs.length} 条记录</span></summary><div>{recentJobs.map((job) => <article key={job.id}><span className={`badge ${job.status === "completed" ? "green" : job.status === "failed" ? "red" : "orange"}`}>{job.status === "completed" ? "已完成" : job.status === "review" ? "待复核" : job.status === "failed" ? "失败" : "处理中"}</span><div><strong>{job.filename}</strong><small>{job.stage}{job.error ? ` · ${job.error}` : ""}</small></div><progress max="100" value={job.progress || 0}/></article>)}</div></details>}

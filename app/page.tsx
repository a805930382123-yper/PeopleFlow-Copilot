...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      <header className="topbar"><div className="topbar-title"><button className="menu-toggle" aria-label="打开导航" aria-expanded={menuOpen} onClick={() => setMenuOpen((value) => !value)}>☰</button><strong className="current-view-title">{activeWorkspace.items.find(([id]) => id === view)?.[2] || pageTitle[view]}</strong></div><div className="top-actions role-actions"><span className="workspace-health"><Badge tone={bootstrapMessage ? "orange" : "green"}>{bootstrapMessage ? "部分数据异常" : loadingData ? "正在同步" : "系统正常"}</Badge></span><label className="workspace-role-switch"><span>演示角色</span><span className="workspace-role-control"><select aria-label="切换演示角色" title="仅切换演示界面，不代表真实账号权限" value={activeWorkspace.id} onChange={(event) => { const space = workspaces.find((item) => item.id === event.target.value); if (space && space.id !== activeWorkspace.id) navigateView(space.items[0][0]); }}>{workspaces.map((space) => <option key={space.id} value={space.id}>{space.label}</option>)}</select><span className="workspace-role-chevron" aria-hidden="true">⌄</span></span></label></div></header>
